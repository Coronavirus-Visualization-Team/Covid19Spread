    ...
    for x in range(counties_info[1]):
        for y in range(len(places_types)): 
            ...
            for i in range(len(info['results'])):
                    ...
                    series = {'State':state, 'County':counties_info[0][x],'Business Name':name, 'Search Tag': searchTag, 'Latitude': lat, 'Longitude': lng}
...
if __name__ == "__main__":
    ...
    with open('', 'r') as resources: #use path of json format .txt file to load data
        ...
        if(not path.exists(outfile)):
            columns = ['ID','State', 'County', 'Business Name', 'Search Tag', 'Latitude', 'Longitude']
